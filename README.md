...
Будем вставлять в таблицу слова из романа М. Митчелл "Унесённый ветром". Для удобства дальнейшей работы текст предварительно отфильтруем следующим образом: уберём цифры, знаки препинания, заменим буквы верхнего регистра на буквы нижнего регистра и напишем каждое слово с новой строчки. В полученном тексте ≈26000 различных слов. Для исследовния хэш-таблицы "перегрузим" её, сделав load-фактор ≈5. То есть её размер сделаем ≈5000 (возьмём простое чило, близкое к 5000 - 5003). 
...
Все столбцы, кроме одного, имеют нулевую высоту, высота оставшегося столбца равна количеству различных слов в исходном тексте.
...
<a name="rol-hash"></a>
### 5. rol_hash
Эта функция считает хэш строки по следующей формуле: $h(n) = rol(h(n-1))$ ^ $str[n]$, где $rol$ - битовое вращение влево, $str[n]$ - n-ая буква строки, ^ - оператор `xor`, $h(0) = 0$. 
```
...
При использовании этой хэш-функции уже заполняются все ячейки таблицы, а также распределение более равномерное.
$D = 3.52$

<a name="ror-hash"></a>
### 6. ror_hash
Эта функция аналогична предыдущей, но вместо `rol` использует функцию `ror` - битовое вращение вправо. Хэш строки считается по формуле: $h(n) = ror(h(n-1))$ ^ $str[n]$.
...
### Вывод
Сравнив полученные значения дисперсии для разных функций, получим следующее (в таблице функции идут от лучшей к худшей)
...
Реализуемые оптимизации будут предполагать знание ассемблера и вставки ассемблерного кода, что значительно ухудшает понимание и переносимость кода. Поэтому будем рассчитывать "коэффициент полезного действия" оптимизации: 

$$K = \frac{a_{отн}}{N} · 1000,$$
...
Характерное время работы программы - ≈ 10-15 секунд.
...
Можно заметить, что этот код можно оптимизировать: например, слово, которое мы ищем в таблице, не меняется. Это значит, что его можно не перезаписывать в регистр `ymm0` при каждом заходе в цикл сравнения строк. Вынесем это за преlелы цикла.
...
Применив все 3 оптимизации, нам удалось ускорить программу в 5 раз.

Рассчитаем для каждой оптимизации "коэффициент полезного действия" $K$, о котором уже говорилось ранее. Напомним, что $$K = \frac{a_{отн}}{N} · 1000,$$
где $N$  - число ассемблерных строк. 

| Версия программы | $a_{абс}$      | $a_{отн}$  | $N$  | $K$  | 
|------------------|----------------|------------|-----------|-------|
| Intrinsic crc32  | $1.00$    |$1.98 ± 0.19$ | $4$         | $495 ± 47$|
| Асм. вставка strcmp  | $1.98 ± 0.19$  | $2.30 ± 0.17$| $11$    | $209 ± 15$|
| find_key на ассемблере | $5.18 ± 0.25$| $1.13 ± 0.03$ | $36$ | $31 ± 1$|
...
## Общие выводы
Проведённая лабораторная работа наглядно показывает, что производительность работы хэш-таблицы сильно зависит от используемой хэш-функции и того, насколько равномерное распределение она даёт. Поэтому для лучшей скорости работы хэш-таблицы важно правильно подбирать хэш-функцию.
