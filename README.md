...
После реализации каждой оптимизации будем рассчитывать абсолютное ускорение $$a_{абс} = \frac{T_i}{T_0},$$ где $T_i$ - время работы версии с i-ой по счёту оптимизацией, $T_0$ - время работы "начальной" версии без оптимизаций, и относительное ускорение  $$a_{отн} = \frac{T_i}{T_{i-1}},$$ где $T_{i-1}$ - время работы предыдущей версии. В случае первой оптимизации эти значения будут совпадать.
